...
export async function clientLoader({ request }: Route.ClientLoaderArgs) {
  try {
    ...
    const { data: weeklyPasses } = await supabase
      .from("pass")
    ...
    const { data: passTypes } = await supabase.from("pass").select("type");

    const { data: hostelRequests } = await supabase
      .from("pass")
      .select(
        `
        id,
    ...
    // Average time from DSA approval to CSO approval (CSO processing time)
    ...
    if (processingData && processingData.length > 0) {
      const totalHours = processingData.reduce((sum, pass) => {
        ...
        return sum + (csoTime - dsaTime) / (1000 * 60 * 60);
      }, 0);
    ...
    return {
      ...
      weeklyData: processWeeklyData(weeklyPasses || []),
      monthlyData: processMonthlyData(monthlyPasses || []),
      passTypeData: processPassTypeData(passTypes || []),
      hostelData: processHostelData(hostelRequests || []),
      error: null,
    };
  } catch (error) {
...
function processWeeklyData(passes: any[]) {
  ...
  const weekData = Array(7)
    ...
    .map((_, i) => {
      ...
      return { day: days[date.getDay()], approved: 0, denied: 0, pending: 0 };
    });

  passes.forEach((pass) => {
    const passDate = new Date(pass.requested_at);
    const dayIndex = (passDate.getDay() + 6) % 7;
...
function processHostelData(passes: any[]) {
  const hostelMap: {
    [key: string]: { hostel: string; requests: number; gender: string };
  } = {};
  ...
  passes.forEach((pass) => {
    ...
    if (hostelName && hostelGender) {
      if (!hostelMap[hostelName]) {
        hostelMap[hostelName] = { hostel: hostelName, requests: 0, gender: hostelGender };
      }
...
export default function CSOAnalyticsPage({ loaderData }: Route.ComponentProps) {
            ...
            <p className="text-xs text-muted-foreground flex items-center gap-1 mt-1">
              {metrics.requestsChange >= 0 ? (
                <>
                  ...
                  <span className="text-emerald-500">+{metrics.requestsChange}%</span>
                </>
              ) : (
                <>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground flex items-center gap-1 mt-1">
              {metrics.approvalRateChange >= 0 ? (
                <>
                  <TrendingUp className="h-3 w-3 text-emerald-500" />
                  <span className="text-emerald-500">+{metrics.approvalRateChange}%</span>
                </>
              ) : (
                <>
                  <TrendingDown className="h-3 w-3 text-red-500" />
                  <span className="text-red-500">{metrics.approvalRateChange}%</span>
                </>
              )}{" "}
              from last month
            </p>
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  />
                  <Bar dataKey="requests" fill="hsl(142, 76%, 36%)" radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
